...
function updatePlayer() {
  if (gameOver || player.position.y <=0) {
    keys[' '] = false; // Reset space key press
    keys['arrowleft'] = false; // Reset left arrow key press
    keys['arrowright'] = false; // Reset right arrow key press
    return; // Don't update player position if game is over or player has fallen off
  }
...
recycleBuildings(player.position.z);
  ...
  fetchHighScore().then(() => {
    if (distance > highScore) {
      highScore = distance;
      document.getElementById("highScoreDisplay").innerText = `High Score: ${highScore}m`;
    }
  });
